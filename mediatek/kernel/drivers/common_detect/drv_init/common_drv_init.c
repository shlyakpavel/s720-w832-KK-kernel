...
#else
	i_ret = -1;
	WMT_DETECT_INFO_FUNC("COMBO chip is not supported, please check kernel makefile or project config, i_ret:%d\n", i_ret);
#endif
...
#else
	i_ret = -1;
	WMT_DETECT_INFO_FUNC("SOC chip is not supported, please check kernel makefile or project config, i_ret:%d\n", i_ret);
#endif
