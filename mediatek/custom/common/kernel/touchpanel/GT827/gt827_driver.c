...
static const struct i2c_device_id tpd_i2c_id[] = {{"gt827",0},{}};
static unsigned short force[] = {0, 0xBA, I2C_CLIENT_END,I2C_CLIENT_END};
static const unsigned short * const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces,};
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO("gt827", (0xBA>>1))};
...
static struct proc_dir_entry *gt827_config_proc = NULL;

static int gt827_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    char *ptr = page;
    ...
    ptr += sprintf( ptr, "==== GT827 config init value====\n" );
    ...
    ptr += sprintf( ptr, "\n" );
    ptr += sprintf( ptr, "==== GT827 charger init config ====\n" );
    for ( i = 0 ; i < CONFIG_LEN ; i++ )
    {
    ...
    ptr += sprintf( ptr, "\n" );
    ptr += sprintf( ptr, "==== GT827 config real value====\n" );
  //  i2c_write_dummy( i2c_client, TPD_HANDSHAKING_START_REG );	
	...
	/* read tpd panel id */
    ptr += sprintf( ptr, "\n" );
    ptr += sprintf( ptr, "==== GT827 panel id value====\n" );
...
static int gt827_config_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
...
static int gt827_check_data(unsigned char *buffer, int count)
{
...
{             
    ...
    //hwPowerOn(MT65XX_POWER_LDO_VGP, VOL_1800, "TP");  
#endif    
    msleep(1);  ///zhaoshaopeng add 20120825
    // set deep sleep off
    mt_set_gpio_mode(GPIO_CTP_RST_PIN, GPIO_CTP_RST_PIN_M_GPIO);
    mt_set_gpio_dir(GPIO_CTP_RST_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ONE);  
   ...
   // i2c_write_dummy( client, TPD_HANDSHAKING_START_REG );
    ...
    gt827_config_proc = create_proc_entry( GT827_CONFIG_PROC_FILE, 0666, NULL);

    if ( gt827_config_proc == NULL )
    {
        TPD_DEBUG("create_proc_entry %s failed\n", GT827_CONFIG_PROC_FILE );
    }
    else 
    {
        gt827_config_proc->read_proc = gt827_config_read_proc;
        gt827_config_proc->write_proc = gt827_config_write_proc;
    }
	#ifdef USE_TRULY_CTP
...
//#endif
    ...
    i2c_write_dummy( client, TPD_HANDSHAKING_END_REG );	
/*
    err = gt827_check_data(cfg_data, 3); 
...
//zhaoshaopeng start
#ifdef TPD_HAVE_BUTTON
struct gt827_touchkey_info
{
	unsigned short y[5];
	unsigned short x[5];
	unsigned int size[5];
};
static int gt827_point_num = 0;
//zhaoshaopeng end
static int touch_event_handler(void *unused)
{
...
{
    TPD_DMESG("MediaTek gt827 touch panel driver init\n");
