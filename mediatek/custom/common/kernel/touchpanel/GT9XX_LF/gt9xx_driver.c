...
extern u8 gup_get_ic_id_msg(struct i2c_client *client);
...
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("gt9xx", (0xBC >> 1))};//(0xBA>> 1)
...
static int gt91xx_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    //<<< lingfude add - 2015/03/10
    /* Touch PID & VID */
    ptr += sprintf(ptr, "\n");
    ptr += sprintf(ptr, "==== GT9XX Version ID ====\n");
    i2c_read_bytes_9xx(i2c_client_point, GTP_REG_VERSION, temp_data, 6);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    u8 cfg_info_group1_lianchuang[]=  CTP_CFG_GROUP1_LIANCHUANG;
	...
	gup_get_ic_id_msg(client);
	if(PRODUCT_ID == PRODUCT_ID_LIANCHUANG)
		send_cfg_buf[0] =  cfg_info_group1_lianchuang;
    ...
    printk("len1=%d,len2=%d,len3=%d,get_len=%d PRODUCT_ID:%d", cfg_info_len[0], cfg_info_len[1], cfg_info_len[2], cfg_len, PRODUCT_ID);
