...
#define GUP_REG_PID_VID2             0x814a
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    printk("GT9xx IC force update flag:0x%x\n", update_msg.force_update);
    ...
    {
		if (memcmp(update_msg.ic_fw_msg.pid, "9006", 4))
		{
			PRODUCT_ID = PRODUCT_ID_LIANCHUANG;
		}
		else
			PRODUCT_ID = PRODUCT_ID_BAOFA;


        if (!memcmp(update_msg.ic_fw_msg.pid, "9111", 4))
        {
            printk("GT9xx IC Mapping Product id:%s\n", update_msg.ic_fw_msg.pid);
            memcpy(update_msg.ic_fw_msg.pid, "9110P", 5);
        }
    }

    update_msg.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH + 4] + (buf[GTP_ADDR_LENGTH + 5] << 8);
    printk("GT9xx IC version id:%04x\n", update_msg.ic_fw_msg.vid);

    return SUCCESS;
}


u8 gup_get_ic_id_msg(struct i2c_client *client)
{
    s32 ret = -1;
    u8  retry = 0;
    u8  buf[16]={0};
    u8  i;
	printk("GT9xx gup_get_ic_id_msg\n");

    ret = gup_get_ic_msg(client, GUP_REG_PID_VID2, buf, 6);

    if (FAIL == ret)
    {
        printk("GT9xx Read hardware info fail.");
        return ret;
    }
	...
	{
		PRODUCT_ID = PRODUCT_ID_BAOFA;
		printk("GT9xx PRODUCT_ID_BAOFA\n");

	}
	else
	{
		PRODUCT_ID =PRODUCT_ID_LIANCHUANG ;
		printk("GT9xx PRODUCT_ID_LIANCHUANG\n");
	}

    printk("GT9xx IC zhuoshineng id:%02x%02x%02x%02x\n", buf[0], buf[1], buf[2], buf[3]);
	printk("GT9xx IC  id:%s\n", buf);
...
void gup_leave_update_mode(void)
{
    ...
    printk("[leave_update_mode]reset chip.");
...
static u8 gup_enter_upadte_judge(st_fw_head *fw_head)
{
    ...
    printk("FILE HARDWARE INFO:%02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    printk("FILE PID:%s", fw_head->pid);
    printk("FILE VID:%04x", fw_head->vid);

    printk("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
              update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
    ...
    {
        printk("Get the same hardware info.");
                ...
                (!memcmp(fw_head->pid, "91XX", 4)))
        {
            ...
            {
                printk("Force none same pid update mode.");
            }
            else
            {
                printk("Get the same pid.");
            }
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    printk("[update_cfg]config file len:%d", file_len);
    printk("[update_cfg]need config len:%d", chip_cfg_len);
    ...
    printk("[update_cfg]Read config from file.");
    ...
    printk("[update_cfg]Delete illgal charactor.");
    ...
    printk("[update_cfg]Ascii to hex.");
    ...
    printk("config:");
    printk(file_config, file_cfg_len);
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
    ...
    else
    {
        ...
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++)
        {
            ...
            if (IS_ERR(update_msg.file))
            {
                ...
                if (IS_ERR(update_msg.file))
                {
                    printk("%3d:Searching file...", i);
                    msleep(1000);
                    continue;
                }
                else
                {
                    printk("Find UPDATE_FILE_PATH_2");
                    break;
                }
            }
            else
            {
                printk("Find UPDATE_FILE_PATH_1");
        ...
        for (; i < GUP_SEARCH_FILE_TIMES; i++)
        {
            ...
            if (IS_ERR(update_msg.cfg_file))
            {
                ...
                {
                    printk("Find CONFIG_FILE_PATH_2");
                    break;
                }
            }
            else
            {
                printk("Find CONFIG_FILE_PATH_1");
                break;
            }
        }
        ...
        if (!IS_ERR(update_msg.cfg_file))
        {
            printk("Got the update config file.");
    ...
    printk("firmware checksum:%x empty_test:%x", (fw_checksum & 0xFFFF), empty_test);
    ...
    while (burn_length < total_length)
    {
        ...
        for (retry = 0; retry < MAX_FRAME_CHECK_TIME; retry++)
        {
            ...
            if (memcmp(&wr_buf[GTP_ADDR_LENGTH], &rd_buf[GTP_ADDR_LENGTH], frame_length))
            {
                ...
                printk("write array:");
                printk(&wr_buf[GTP_ADDR_LENGTH], frame_length);
                printk("read array:");
                printk(&rd_buf[GTP_ADDR_LENGTH], frame_length);
                continue;
            }
            else
            {
...
static u8 gup_recall_check(struct i2c_client *client, u8 *chk_src, u16 start_rd_addr, u16 chk_length)
{
    ...
    while (recall_length < chk_length)
    {
        ...
        if (memcmp(&rd_buf[GTP_ADDR_LENGTH], &chk_src[recall_length], frame_length))
        {
            ...
            printk(&chk_src[recall_length], frame_length);
            printk("recall array:");
            printk(&rd_buf[GTP_ADDR_LENGTH], frame_length);
            return FAIL;
        }
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
    ...
    printk("[burn_dsp_isp]Begin burn dsp isp---->>");
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
    ...
    printk("[burn_fw_ss51]Begin burn ss51 firmware---->>");
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
    ...
    printk("[burn_fw_dsp]Begin burn dsp firmware---->>");
    //step1:alloc memory
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
    ...
    printk("[burn_fw_boot]Begin burn bootloader firmware---->>");
    ...
    printk("[burn_fw_boot]step5:release ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x00);
    ...
    printk("[burn_fw_boot]step9:enable download DSP code ");
    ...
    printk("[burn_fw_boot]step10:release ss51 & hold dsp");
...
static u8 gup_download_fw_ss51(struct i2c_client *client)
{
    ...
    //step1:alloc memory
    printk("[download_fw_ss51]step1:alloc memory");
    ...
    printk("[download_fw_ss51]step2:clear control flag");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, 0x00);
    ...
    //step3:disable wdt,clear cache enable
    printk("[download_fw_ss51]step3:disable wdt,clear cache enable");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__TMR0_EN, 0x00);
    ...
    //step4:hold ss51 & dsp
    printk("[download_fw_ss51]step4:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    ...
    //step5:set boot from sram
    printk("[download_fw_ss51]step5:set boot from sram");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOTCTL_B0_, 0x02);
    ...
    //step6:software reboot
    printk("[download_fw_ss51]step6:software reboot");
    ret = gup_set_ic_msg(client, _bWO_MISCTL__CPU_SWRST_PULSE, 0x01);
    ...
    printk("[download_fw_ss51]step7:load ss51 firmware section 1 file data");
...
static u8 gup_download_fw_dsp(struct i2c_client *client)
{
    ...
    //step1:alloc memory
    printk("[download_fw_dsp]step1:alloc memory");
    ...
    //step2:load firmware dsp
    printk("[download_fw_dsp]step2:load firmware dsp");
    ret = gup_load_section_file(fw_dsp, 4 * FW_SECTION_LENGTH, FW_DSP_LENGTH);
    ...
    //step3:select bank2
    printk("[download_fw_dsp]step3:select bank2");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x02);
...
download:
    ...
    //-- set scramble
    printk("[download_fw_dsp]step5:set scramble");
    ...
    //-- hold ss51 & dsp
    printk("[download_fw_dsp]step4:hold ss51 & dsp");
    ...
    printk("[fw_download_proc] BOOT Status %02X\n", buf[2]);
...
#if 0
    printk("[fw_download_proc]send config.");
    ret = gtp_send_cfg(guitar_client);
