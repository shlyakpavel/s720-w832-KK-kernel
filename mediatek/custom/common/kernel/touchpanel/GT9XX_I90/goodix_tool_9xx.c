...
//        || !strncmp(IC_TYPE, "GT813", 5))
    if (strncmp(IC_TYPE_9xx, "GT8110", 6) && strncmp(IC_TYPE_9xx, "GT8105", 6)
            && strncmp(IC_TYPE_9xx, "GT801", 5) && strncmp(IC_TYPE_9xx, "GT800", 5)
            && strncmp(IC_TYPE_9xx, "GT801PLUS", 9) && strncmp(IC_TYPE_9xx, "GT811", 5)
            && strncmp(IC_TYPE_9xx, "GTxxx", 5))
    {
...
s32 init_wr_node_9xx(struct i2c_client *client)
{
    s32 i;
    ...
    if (i)
    {
        DATA_LENGTH_9xx = i * DATA_LENGTH_UINT + GTP_ADDR_LENGTH;
        GTP_INFO("Applied memory size:%d.", DATA_LENGTH_9xx);
    }
    else
    {
...
#else
    if(proc_create(GOODIX_ENTRY_NAME, 0666, NULL, &gt_tool_fops)== NULL)
	{
...
static s32 goodix_tool_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
    ...
    {
        memcpy(IC_TYPE_9xx, cmd_head.data, cmd_head.data_len);
        register_i2c_func();
