...
static int breakDownSteps(int max_step, int interval, int CurrentPos, int TargetPos)
{
    ...
    OV8835AFDB("[OV8835AF] break down steps from=%d, to=%d \n", CurrentPos, TargetPos);
    ...
    while( abs(CurrentPos - TargetPos) > max_step ){
        ...
        OV8835AFDB("tmpTargetPos=%d\n", tmpTargetPos);
        if(s4OV8835AF_WriteReg(tmpTargetPos) == 0)
        {
            CurrentPos = tmpTargetPos;
            OV8835AFDB("CurrentPos=%d\n", CurrentPos);
        }
        else
        {
        ...
        OV8835AFDB("start delay \n");
        msleep(interval);
        OV8835AFDB("after delay \n");
    }

    OV8835AFDB("Last step from=%d, to=%d \n", CurrentPos, TargetPos);
    if(s4OV8835AF_WriteReg(TargetPos) == 0)
    {
          CurrentPos = TargetPos;
    }
    else
...
inline static int moveOV8835AF(unsigned long a_u4Position)
{
    ...
    {
        OV8835AFDB("[OV8835AF] in small stepLeave breakdown \n");
